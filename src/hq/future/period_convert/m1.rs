    ...
    for (breed, time_range) in time_range_hmap {
        ...
        for idx in 0..open_times.len() {
            if idx == 0 {
                ...
                match hhmmss.hhmmss {
                    9_00_00 => {
                        hhmm_time_map.insert(859u16, NaiveTime::from_hms_opt(9, 1, 0).unwrap());
                    },
                    9_30_00 => {
...
impl Converter1m {
    ...
    /// time 为自然时间
    pub fn convert(breed: &str, dt: &NaiveDateTime) -> Result<NaiveDateTime, PeriodConvertError> {
        let date = dt.date();
        let hms = Hms::from(dt);
        if hms.hhmmss == 0 {
...
mod tests {
    ...
    async fn test_1m(breed: &str, results: &[(&str, &str)]) {
        ...
        for (source, target) in results {
            ...
            let time_1m = Converter1m::convert(breed, &dt).unwrap();
